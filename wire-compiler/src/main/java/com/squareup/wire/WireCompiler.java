...
public final class WireCompiler {
  ...
  static WireCompiler create(CommandLineOptions options) throws WireException {
    ...
    return new WireCompiler(options, fs, loader, JavaGenerator.IO.DEFAULT,
        new ConsoleWireLogger(options.quiet));
  }

  WireCompiler(CommandLineOptions options, FileSystem fs, Loader loader,
      JavaGenerator.IO javaGeneratorIo, WireLogger logger) throws WireException {
    this.fs = fs;
  ...
  public void compile() throws IOException {
    List<String> sourceFileNames = options.sourceFileNames;
    List<Path> sourcePaths = new ArrayList<>(sourceFileNames.size());
    for (String sourceFileName : sourceFileNames) {
      Path path = fs.getPath(sourceFileName);
      sourcePaths.add(path);
    }

    Schema schema = loader.load(sourcePaths);
